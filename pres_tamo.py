...
# --- 1. BASE DE DATOS PROFESIONAL (Persistencia Total) ---
def conectar_db():
    conn = sqlite3.connect('cartera_lesthy_definitiva_final.db')
    c = conn.cursor()
    # Tabla Principal: Registra todo el ciclo de vida del préstamo
    ...
    # Tabla de Configuración: Tu número personal
...
menu = st.sidebar.radio("NAVEGACIÓN PRINCIPAL", [
    ...
    "✅ Gestión de Cobros (WhatsApp)", 
    "🚨 LISTA NEGRA (Automática)", 
    "🏆 Historial de Ganancias",
    "🔧 Editor Maestro e Inventario"
])
...
if menu == "🔥 Nuevo Préstamo / Vista Previa":
    ...
    if m > 0:
        ...
        valor_c = total_p / c
        st.markdown("---")
        st.subheader("📊 Análisis de Riesgo y Vista Previa")
        ...
        st.info(f"✅ Detalle: {int(c)} cuotas de ${valor_c:,.0f} ({mov})".replace(",","."))

        st.write("**Calificación Inicial (Selección Manual):**")
        cb, cm = st.columns(2)
        v_b = cb.checkbox("✅ Marcar como BUEN CLIENTE", value=False)
        v_m = cm.checkbox("🚨 Marcar como CLIENTE MALO (Moroso)", value=False)

        if st.button("🚀 ACTIVAR PRÉSTAMO PERMANENTE"):
            if v_b or v_m:
                ...
                conn.commit()
                st.success(f"✔️ ¡Préstamo de {n} activado exitosamente!")
            else: st.warning("⚠️ Debes elegir una calificación inicial.")

# --- MÓDULO B: COBROS + REENVÍO A TU WHATSAPP ---
elif menu == "✅ Gestión de Cobros (WhatsApp)":
    st.subheader("📈 Cobranza Activa e Inteligente")
    ...
    for _, row in df.iterrows():
        with st.expander(f"👤 {row['nombre']} | Cuota {row['cuotas_pagadas']+1} | Próximo: {row['proxima_fecha']}"):
            if st.button(f"REGISTRAR PAGO #{row['cuotas_pagadas']+1}", key=f"btn_{row['id']}"):
                ...
                # Inteligencia de Mora (Regla de los 5 fallos)
                ...
                # Calcular siguiente fecha según movilidad
                ...
                # GENERAR ENLACE HACIA TU WHATSAPP PERSONAL
                msg = f"*🧾 REPORTE DE COBRO - LESTHY_BOT*\n\n*Cliente:* {row['nombre']}\n*WhatsApp:* {row['telefono_cliente']}\n*Cuota:* {nuevas_p}/{row['cuotas_totales']}\n*Estado:* {estado}\n*Malos Pagos:* {nuevos_malos}/5\n\n_Favor reenviar información al cliente._".replace(",",".")
                link_mi_wa = f"https://wa.me/{mi_numero}?text={urllib.parse.quote(msg)}"
                
                st.markdown(f'''<a href="{link_mi_wa}" target="_blank">
                    <button style="background-color:#25D366; color:white; border:none; padding:12px; border-radius:10px; cursor:pointer; width:100%;">
                        🟢 OBTENER REPORTE EN MI WHATSAPP
                    </button></a>''', unsafe_allow_html=True)
                st.rerun()

# --- MÓDULO C: EDITOR MAESTRO (TOTAL) ---
elif menu == "🔧 Editor Maestro e Inventario":
    st.subheader("🛠️ Panel de Edición y Control Total")
    ...
    if id_edit in df_full['id'].values:
        cliente = df_full[df_full['id'] == id_edit].iloc[0]
        with st.form("form_master_edit"):
            st.write(f"📝 Editando: **{cliente['nombre']}**")
            c1, c2 = st.columns(2)
            en = c1.text_input("Nombre", cliente['nombre'])
            et = c1.text_input("WhatsApp Cliente", cliente['telefono_cliente'])
            em = c2.number_input("Capital Base", value=float(cliente['monto_base']))
            ec = c2.number_input("Cuotas Totales", value=int(cliente['cuotas_totales']))
            ep = c2.number_input("Cuotas Pagadas", value=int(cliente['cuotas_pagadas']))
            est = c1.selectbox("Reputación", ["Buen Cliente", "Cliente Moroso", "Finalizado"], 
                               index=["Buen Cliente", "Cliente Moroso", "Finalizado"].index(cliente['reputacion']))
            emp = c2.number_input("Malos Pagos (Strikes)", value=int(cliente['malos_pagos']))
            
            if st.form_submit_button("💾 GUARDAR CAMBIOS EN BASE DE DATOS"):
                conn.cursor().execute("""UPDATE registros SET nombre=?, telefono_cliente=?, monto_base=?, 
                    ...
                    (en, et, em, ec, ep, emp, est, id_edit))
                conn.commit()
...
# --- MÓDULOS DE ANÁLISIS ---
    ...
    st.subheader("🔴 Lista de Morosos Críticos (+5 Atrasos)")
    df_negra = pd.read_sql_query("SELECT * FROM registros WHERE reputacion = 'Cliente Moroso'", conn)
    st.error("⚠️ Estas personas han superado el límite de confianza.")
    st.table(df_negra)

elif menu == "🏆 Historial de Ganancias":
    st.subheader("🏁 Cierre de Negocios Exitosos")
    df_ganado = pd.read_sql_query("SELECT * FROM registros WHERE reputacion = 'Finalizado'", conn)
    if not df_ganado.empty:
        neto = df_ganado['total_pagar'].sum() - df_ganado['monto_base'].sum()
        st.metric("Ganancia Neta Total", f"${neto:,.0f}".replace(",","."), delta="💰")
        st.dataframe(df_ganado)
    else: st.info("No hay préstamos finalizados todavía.")
